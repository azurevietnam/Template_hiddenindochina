...
<?php if (isset($visforms->errors) && is_array($visforms->errors) && count($visforms->errors) > 0)
{
    ...
    foreach ($visforms->errors as $error)
    {
        echo '<p>' . $error . '</p>';
    }
    echo '</div>';
...
?>
        ...
        jQuery('.captcharefresh<?php echo $visforms->id; ?>').bind({
            'click' : function() {
                ...
                {
                    jQuery('#captchacode<?php echo $visforms->id; ?>').attr('src', 'index.php?option=com_visforms&task=visforms.captcha&sid=' + Math.random()+ '&id=<?php echo $visforms->id; ?>');
                }
            }
        });	
});
